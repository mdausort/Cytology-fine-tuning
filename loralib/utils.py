    ...
    if args.encoder == "vision" or args.encoder == "both":
        ...
        if args.model_name == "uni":
            ...
            for i, block in enumerate(vision_encoder.blocks):
                ...
                if i in indices:
                    for name, submodule in block.named_children():
                        if "Attention" in str(type(submodule)):
                            new_attention_lora = SimpleAttentionLoRA(
                                submodule,
                                enable_lora=args.params,
                                r=args.r,
                                lora_alpha=args.alpha,
                            ...
                            setattr(block, name, new_attention_lora)
                            list_lora_layers.append(new_attention_lora)
            ...
            vision_encoder = clip_model.vit.encoder
            for i, block in enumerate(vision_encoder.layer):
                for name, submodule in block.attention.attention.named_children():
                    for item in args.params:
                        if item == "q" and name == "query":
                            new_attention_lora = LinearLoRA(
                                submodule,
                                r=args.r,
                                lora_alpha=args.alpha,
                                dropout_rate=args.dropout_rate,
                            )
                            setattr(block.attention.attention, name, new_attention_lora)
                            list_lora_layers.append(new_attention_lora)
                        elif item == "k" and name == "key":
                            new_attention_lora = LinearLoRA(
                                submodule,
                                r=args.r,
                                lora_alpha=args.alpha,
                            ...
                            setattr(block.attention.attention, name, new_attention_lora)
                            list_lora_layers.append(new_attention_lora)
                for name, submodule in block.attention.output.named_children():
                    if "o" in args.params and name == "dense":
                        new_attention_lora = LinearLoRA(
                            submodule,
                            r=args.r,
                            lora_alpha=args.alpha,
