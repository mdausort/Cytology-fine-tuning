...
class SimpleAttentionLoRA(nn.Module):
    def __init__(
        self,
        existing_attn: Attention,
        enable_lora: list = ["q", "k", "v", "o"],
        r: int = 0,
        lora_alpha: int = 1,
        dropout_rate: float = 0.0,
        **kwargs,
        ...
        self.embed_dim = (
            existing_attn.qkv.in_features
        )  # Assuming in_features corresponds to the embed_dim

        self.q_proj = nn.Linear(
            self.embed_dim, self.embed_dim, bias=existing_attn.qkv.bias is not None
        )
        self.k_proj = nn.Linear(
            self.embed_dim, self.embed_dim, bias=existing_attn.qkv.bias is not None
        )
        ...
        self.q_norm = existing_attn.q_norm
        self.k_norm = existing_attn.k_norm
        self.attn_drop = existing_attn.attn_drop
        self.proj = nn.Linear(
            self.embed_dim, self.embed_dim, bias=existing_attn.qkv.bias is not None
        )
        self.proj_drop = existing_attn.proj_drop

        # Initialize parameters
        with torch.no_grad():

            # Extract the existing weights and biases
            existing_weight = existing_attn.qkv.weight.data
            existing_bias = (
                existing_attn.qkv.bias.data
                if existing_attn.qkv.bias is not None
                else None
            )
            ...
            # Initialize proj
            self.proj.weight.data.copy_(existing_attn.proj.weight.data)
            if self.proj.bias is not None:
                self.proj.bias.data.copy_(existing_attn.proj.bias.data)
